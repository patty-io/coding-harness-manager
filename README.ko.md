...
4. **Sync from library…**를 클릭합니다. 활성화된 모든 MCP 서버가 기본으로
   선택되므로, 일부만 동기화하려면 체크를 해제합니다.
5. 대상 하네스를 선택하고 적용 전에 네이티브 계획을 검토합니다.
...
프로필과 구성 세트는 워크플로를 정리하는 동안 릴리스 플래그로 보호되며
기본적으로 숨겨져 있습니다. 사용하려면 **Settings → Experimental features
→ Profiles and configuration sets**를 활성화하세요. 설정은
`~/.coding-harness-manager/feature-flags.json`의 `profilesAndSets`에
저장됩니다.
