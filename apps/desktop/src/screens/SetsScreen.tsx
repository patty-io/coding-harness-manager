...
type ItemType = "model_route" | "mcp_server" | "skill" | "launch_profile";

const ITEM_LABELS: Record<ItemType, string> = {
  model_route: "Model",
  mcp_server: "MCP server",
  skill: "Skill",
  launch_profile: "Preset",
};
...
function SetCard({
  ...
  const previewMutation = useMutation({
    mutationFn: () => applySetPreview(set.id, installationId),
    onSuccess: setPreview,
  });
  const apply = useMutation({
    mutationFn: () => applySet(set.id, installationId, mode),
  ...
  return (
    <li className="rounded border border-slate-700 bg-slate-800 p-4">
      ...
      <div className="mt-4 rounded border border-slate-700/80 bg-slate-900/40 p-3">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          <select value={installationId} onChange={(e) => setInstallationId(e.target.value)} className="rounded border border-slate-600 bg-slate-900 px-2 py-1 text-sm">
        ...
        {preview && (
          <div className="mt-3 text-xs text-slate-300">
            ...
            {preview.actions.length > 0 && <ul className="mt-1 list-inside list-disc text-slate-400">{preview.actions.map((action, index) => <li key={`${action.kind}:${action.identity}:${index}`}>{action.action} {action.kind} {action.identity}</li>)}</ul>}
...
export default function SetsScreen() {
  ...
  const choices = useMemo(() => ({
    ...
    launch_profile: (profiles.data ?? []).map((p) => ({ id: p.id, label: `${p.name} · ${p.harnessType}` })),
  }), [routes.data, mcp.data, skills.data, profiles.data]);

  return (
    <div>
      <div className="flex items-center justify-between"><div><h1 className="text-2xl font-bold">Configuration sets</h1><p className="mt-1 text-sm text-slate-400">Reusable bundles of models, MCP servers, skills and presets. Preview before applying to a harness.</p></div><button type="button" onClick={() => setShowCreate((v) => !v)} className="rounded bg-blue-600 px-3 py-1 text-sm text-white hover:bg-blue-500">{showCreate ? "Cancel" : "Create set"}</button></div>
